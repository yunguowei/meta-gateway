...
IPTABLES_SAVED="/var/cache/iptables_saved"
IP6TABLES_SAVED="/var/cache/ip6tables_saved"
FIREWALL_CONF="/etc/config/firewall"
FIREWALL_CONF_MD5SUM_SAVED="/var/cache/firewall_config_md5sum"
...
fw_start() {
	fw_is_loaded && {
       ...
       # If firewall confg file has changed, reload it.
	[ -e "$FIREWALL_CONF_MD5SUM_SAVED" ] && {
	        oldmd5sum=`cat $FIREWALL_CONF_MD5SUM_SAVED`
		newmd5sum=`md5sum $FIREWALL_CONF | awk '{print $1}'`
		[  "$oldmd5sum"  !=  "$newmd5sum"  ] && {
			# Remove iptables stored files
			[ -e "$IPTABLES_SAVED" ] && rm -rf $IPTABLES_SAVED
			[ -e "$IP6TABLES_SAVED" ] && rm -rf $IP6TABLES_SAVED
			echo $newmd5sum > $FIREWALL_CONF_MD5SUM_SAVED
		}
	} || {
		md5sum $FIREWALL_CONF | awk '{print $1}' > $FIREWALL_CONF_MD5SUM_SAVED
	}

	# If there is iptables saved files, start firewall in fast mode.
	[ -e "$IPTABLES_SAVED" -a -e "$IP6TABLES_SAVED" ] && {
		echo -n "Starting firewall: "
		iptables-restore < $IPTABLES_SAVED && \
		ip6tables-restore < $IP6TABLES_SAVED  && {
			echo "[ OK ]"
	...
	# Save iptables rules for fast start next time.
	iptables-save > $IPTABLES_SAVED
	ip6tables-save > $IP6TABLES_SAVED
}
...
fw_stop() {
	...
	for z in $z; do
		...
		[ "$i" == 1 ] && {
			fw del i m FORWARD zone_${z}_MSSFIX 2>/dev/null
			fw del i m zone_${z}_MSSFIX 2>/dev/null
		}
	done

	# Remove all user-defined chains in mangle table
	# Because the above code will not be run because
	# core state section is not set when start via iptables restore
	fw del i m
