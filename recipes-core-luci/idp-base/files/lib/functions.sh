...
run_background() {
	...
	shift
	$@ &
	mkdir -p /tmp/.boot-cache && echo $! > /tmp/.boot-cache/$id.pid
}

wait_background() {
	while kill -0 `cat /tmp/.boot-cache/$1.pid 2>/dev/null` 2>/dev/null ; do
		sleep 1;
	done
	rm -f /tmp/.boot-cache/$1.pid
}
